...
import { getCustomBlog, type BlogMetadata } from "@/content/blog/registry";
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  // Check custom blog registry first
  const customBlog = getCustomBlog(slug);
  if (customBlog) {
    const h = await headers();
    const host = h.get("host") || "www.royaljubilant.com";
    const proto = h.get("x-forwarded-proto") || "https";
    const canonicalUrl = `${proto}://${host}/market-insights/${slug}`;
    const description = customBlog.metadata.excerpt?.slice(0, 160) || customBlog.metadata.title;
    return {
      title: `${customBlog.metadata.title} | Royal Jubilant Real Estate`,
      description,
      alternates: { canonical: canonicalUrl },
      openGraph: {
        title: customBlog.metadata.title,
        description,
        url: canonicalUrl,
        siteName: "Royal Jubilant Real Estate",
        type: "article",
        locale: "en_AE",
        images: customBlog.metadata.image ? [{ url: customBlog.metadata.image, width: 1200, height: 630 }] : [],
        publishedTime: new Date(customBlog.metadata.date).toISOString(),
        authors: [customBlog.metadata.authorName],
      },
      twitter: {
        card: "summary_large_image",
        title: customBlog.metadata.title,
        description,
        images: customBlog.metadata.image ? [customBlog.metadata.image] : [],
      },
    };
  }
...
export default async function BlogDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  // ═══════════════════════════════════════════════════════════════════════
  // CHECK CUSTOM BLOG REGISTRY FIRST
  // If a custom premium blog file exists for this slug, render it.
  // ═══════════════════════════════════════════════════════════════════════
  const customBlog = getCustomBlog(slug);
  if (customBlog) {
    const { metadata: m, Component: ArticleBody } = customBlog;

    // Fetch related + recommended from DB
    let related: any[] = [];
    try { related = await db.blogPost.findMany({ where: { published: true, category: m.category, title: { not: m.title } }, take: 3, orderBy: { date: "desc" } }); } catch {}
    ...
    const proto = h.get("x-forwarded-proto") || "https";
    const canonicalUrl = `${proto}://${host}/market-insights/${slug}`;
    const formattedDate = new Date(m.date).toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" });

    const articleSchema = {
      "@context": "https://schema.org", "@type": "Article",
      headline: m.title, description: m.excerpt, image: m.image,
    ...
    return (
      <div className="min-h-screen bg-white">
        ...
        <header className="relative min-h-[70vh] flex items-end overflow-hidden bg-[#0A1F44]">
          ...
          <div className="relative z-10 w-full pb-12 lg:pb-16">
            <div className="container mx-auto px-4 lg:px-6 max-w-4xl">
              <span className="inline-flex px-3 py-1 rounded-full bg-[#C9A961] text-[#0A1F44] text-[10px] font-bold tracking-luxury uppercase mb-5">{m.category}</span>
              <h1 className="font-serif text-3xl sm:text-4xl lg:text-5xl xl:text-6xl font-medium text-white leading-[1.1] mb-5 max-w-3xl" style={{ textShadow: "0 2px 30px rgba(0,0,0,0.5)" }}>{m.title}</h1>
              <p className="text-base lg:text-lg text-white/80 leading-relaxed max-w-2xl mb-6 font-light">{m.excerpt}</p>
              <div className="flex items-center gap-4 sm:gap-6 flex-wrap">
                <div className="flex items-center gap-2.5">
                  <div className="size-10 rounded-full bg-gradient-to-br from-[#C9A961] to-[#A68A3F] flex items-center justify-center text-[#0A1F44] text-sm font-bold flex-shrink-0 ring-2 ring-white/20">{m.authorName?.charAt(0) || "R"}</div>
                  <div>
                    <p className="text-sm font-medium text-white">{m.authorName}</p>
                    <p className="text-[10px] text-white/50">{m.authorRole || "Royal Jubilant Real Estate"}</p>
                  </div>
                </div>
                <div className="flex items-center gap-4 text-xs text-white/60">
                  <span className="flex items-center gap-1.5"><Calendar className="size-3.5" /> {formattedDate}</span>
                  <span className="flex items-center gap-1.5"><Clock className="size-3.5" /> {m.readTime}</span>
                </div>
              </div>
            </div>
          </div>
        ...
        {/* ARTICLE BODY — max 800px reading width */}
        <div className="container mx-auto px-4 lg:px-6 py-12 lg:py-20">
          <div className="max-w-3xl mx-auto">
            <article style={{ maxWidth: "800px" }}>
              ...
              <div className="mt-12 pt-8 border-t border-[#E5E7EB] flex items-center justify-between gap-4 flex-wrap">
                <div className="flex items-center gap-3">
                  ...
                  <ShareButton title={m.title} description={m.excerpt} size="sm" />
                </div>
                <BackToTopButton />
              </div>
            </article>
            ...
            <div className="mt-12 p-8 rounded-2xl bg-[#0A1F44] text-center">
              <div className="size-16 rounded-full bg-gradient-to-br from-[#C9A961] to-[#A68A3F] flex items-center justify-center text-[#0A1F44] text-xl font-bold mx-auto mb-4 ring-4 ring-white/10">{m.authorName?.charAt(0) || "R"}</div>
              <h3 className="font-serif text-lg font-medium text-white mb-1">{m.authorName}</h3>
              <p className="text-xs text-[#C9A961] uppercase tracking-luxury mb-3">{m.authorRole || "Royal Jubilant Real Estate"}</p>
              <p className="text-sm text-white/60 max-w-md mx-auto leading-relaxed mb-5">{m.authorName} is a RERA-certified property consultant at Royal Jubilant Real Estate LLC, specialising in Dubai's luxury residential and investment property market.</p>
              <Link href="/#/agents" className="text-xs text-[#C9A961] hover:underline font-medium">View all advisors →</Link>
            </div>
          </div>
        </div>
        ...
        {related.length > 0 && (
          <section className="bg-[#F9FAFB] border-t border-[#E5E7EB] py-16 lg:py-20">
            <div className="container mx-auto px-4 lg:px-6 max-w-5xl">
              <h2 className="font-serif text-2xl lg:text-3xl font-medium text-[#0A1F44] mb-8">Related Articles</h2>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8 min-w-0">
                {related.map((rp) => (
                  <Link key={rp.id} href={`/market-insights/${slugify(rp.title)}`} className="group min-w-0">
        ...
        {recommendedProps.length > 0 && (
          <section className="bg-white border-t border-[#E5E7EB] py-16 lg:py-20">
            <div className="container mx-auto px-4 lg:px-6 max-w-5xl">
              <h2 className="font-serif text-2xl lg:text-3xl font-medium text-[#0A1F44] mb-8">Featured Properties</h2>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8 min-w-0">
                {recommendedProps.map((p) => {
                  const imgs = (() => { try { return JSON.parse(p.images || "[]"); } catch { return []; } })();
                  return (
                    <Link key={p.id} href={`/properties/${p.slug || p.reference || p.id}`} className="group bg-white rounded-2xl overflow-hidden border border-[#E5E7EB] hover:shadow-xl transition-shadow min-w-0">
        ...
        <section className="bg-[#0A1F44] py-16 lg:py-20 relative overflow-hidden">
          ...
          <div className="container mx-auto px-4 lg:px-6 max-w-3xl text-center relative">
            <h2 className="font-serif text-2xl lg:text-4xl font-medium text-white mb-4">Speak to a Royal Jubilant Advisor</h2>
            <p className="text-white/60 text-sm lg:text-base mb-8 max-w-xl mx-auto leading-relaxed">Our RERA-certified advisors provide personal, research-led counsel on Dubai's luxury property market — from first viewing to title deed handover.</p>
            <div className="flex items-center justify-center gap-3 flex-wrap">
              <a href="https://wa.me/971524942329" target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 px-7 py-3.5 rounded-full bg-[#25D366] hover:bg-[#1FB855] text-white text-sm font-semibold transition-colors shadow-lg"><MessageCircle className="size-4" /> WhatsApp an Advisor</a>
              <a href="tel:+97143278401" className="inline-flex items-center gap-2 px-7 py-3.5 rounded-full bg-[#C9A961] hover:bg-[#D4B875] text-[#0A1F44] text-sm font-semibold transition-colors shadow-lg"><Phone className="size-4" /> Book a Consultation</a>
              <Link href="/#/contact" className="inline-flex items-center gap-2 px-7 py-3.5 rounded-full bg-white/10 hover:bg-white/20 border border-white/20 text-white text-sm font-semibold transition-colors"><Mail className="size-4" /> Contact Us</Link>
            </div>
          </div>
        </section>
        ...
        <footer className="bg-[#0A1F44] border-t border-white/10 py-6">
          <div className="container mx-auto px-4 lg:px-6 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-white/40">
            ...
            <Link href="/#/blog" className="text-[#C9A961] hover:underline inline-flex items-center gap-1"><ArrowLeft className="size-3" /> Back to Market Insights</Link>
          </div>
        </footer>
      </div>
    );
  ...
  // ═══════════════════════════════════════════════════════════════════════
  // FALL BACK TO DATABASE (admin portal blogs)
  // ═══════════════════════════════════════════════════════════════════════
  const post = await findPostBySlug(slug);
  if (!post) notFound();
